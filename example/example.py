...
def example():
    stan_model_path = '../acoustic_gps/stan_models/' # Models in Stan code for Bayesian inference
    compiled_stan_model_path = './compiled/'
    montecarlo_chains = 4
    ...
    # sound field to reconstruct
    number_meas_reps = 1 # number of measurement repetitions (maybe several sine sweeps?)
    field_grid_rows = 26 # field discretization
    ...
    measured_sound_field = noisy_sound_field[:, 0, i_measured] # Single frequency
    ...
    if compile is True:
        agp.utils.compile_model(
                                model_name=kernel,
                                model_path=stan_model_path, 
                                compiled_save_path=compiled_stan_model_path
                                )
    
    # Data used by Stan for Monte Carlo sampling. This is common for all model.
    data = dict(
                x=measured_locations,
                N_meas=measured_sound_field.shape[-1],
                N_reps=number_meas_reps,
                y=np.concatenate(
                    (measured_sound_field.real, measured_sound_field.imag), axis=-1),
                k=(2 * np.pi * sound_field_parameters['f'][0] / sound_field_parameters['c']),
                delta=1e-10,
                Sigma=(sound_field_parameters['noise_std'][0]**2)/2 * np.eye(2*measured_sound_field.shape[-1])
            )
    ...
    posterior_samples, posterior_summary = agp.mc_sampling(
            compiled_stan_model_path + kernel + ".pkl",
            data=data,
            pars=stan_pars,
    ...
    predicted_mean = bivariate_predicted_mean[:, :all_locations.shape[0]] + 1j*bivariate_predicted_mean[:, all_locations.shape[0]:]
    Krr, Kri, Kir, Kii = agp.utils.split_covariance_in_blocks(bivariate_predicted_covariance)
    predicted_covariance = Krr+Kii+1j*(Kir-Kri)
    predicted_variance = np.diag(predicted_covariance[0])


    # Show reconstruction
    plot_reconstruction(all_locations, measured_locations, true_sound_field[0].real, np.median(predicted_mean,axis=0).real, np.abs(predicted_variance))
